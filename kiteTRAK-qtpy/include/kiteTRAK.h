...
// Switch Mode Menu
/*
Future change idea, add draw menu items as apart of each mode
instead of all new items for each menu/mode

I.e., if (disp_mode == MODE_STICKBOX && !disp_menu) draws main display
and   if (disp_mode == MODE_STICKBOX && disp_menu) draws menu for mode
*/

void display_menu(int disp_mode, int Dl, int Dr) {
    ...
    switch (disp_mode) {
        case MODE_STICKBOX:
            menu_text = "Sticks";
            // Analog stick
            display.drawCircle(46, 19, 14, 1);
            display.drawCircle(46, 19, 10, 1);
            display.drawCircle(46, 19, 6, 1);
            // C stick
            display.drawCircle(83, 24, 14, 1);
            display.fillCircle(83, 24, 7, 1);
            display.drawBitmap(81, 21, cstick_c_bmp, 4, 7, SSD1306_BLACK);
            break;
        case MODE_BUTTONS_DEBUG:
            menu_text = "Buttons";
            break;
    }
    ...
    display.setCursor(text_x, text_y + 14); // Adjust to bottom of border
