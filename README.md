...
## How it Works
A QtPy RP2040 microcontroller is added to the controller to read the PhobGCC state and drive the 128x64 OLED display. The PhobGCC and QtPy RP2040 utilize DMA-based SPI communication for extremely fast comm speeds without interrupting the PhobGCC and maintaining reliable controller functionality. The QtPy RP2040 and OLED display are powered via the 5V line, replacing the rumble motor. The current draw is measured to be 30-40mA, which is about the same current draw as the rumble motor during steady-state use.
...
* Olympia removable gateplate: 8001 Resin Transparent with Oil Spraying or 9600R White Resin with General Sanding
    * Note: You will receive a warning for a print thickness <0.8mm with JLC, you can accept this warning
